...
class FeatDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with torch.no_grad():
            feats1 = self.pad_or_sample(
                torch.tensor(feats1), self.num_feats, self.feat_len
            )
            feats2 = self.pad_or_sample(
                torch.tensor(feats2), self.num_feats, self.feat_len
            )

        assert (
            len(feats1.shape) == 2
            and feats1.shape[0] == self.num_feats
            and feats1.shape[1] == self.feat_len
        ), f"{feats1.shape=}"
        assert (
            len(feats2.shape) == 2
            and feats2.shape[0] == self.num_feats
            and feats2.shape[1] == self.feat_len
        ), f"{feats2.shape=}"
...
def check_file(f):
    ...
    assert os.path.getsize(f) > 800, f"{f} is broken!"


def get_pat_dict(cfg, num_cores=8):
    pat_dict = {}
    print(f'FMs: {cfg["general"]["fms"]}')
    # for c in tqdm(cfg["general"]["feat_cohorts"]):
    for c in tqdm(
        os.listdir(
            os.path.join(cfg["general"]["feat_base_paths"][0], cfg["general"]["fms"][0])
        ),
        leave=False,
    ):
            ...
            for feat_base_path in cfg["general"]["feat_base_paths"]:
                ...
                feat_path = os.path.join(
                    ...
                    [
                        ...
                        if "stamp" in f and os.path.isdir(os.path.join(feat_path, f))
                    ][0],
                )
                feat_files = glob(os.path.join(feat_path, "*.h5"))
                assert (
                    len(feat_files) > 0
                ), f"couldnt find any feat files in path {feat_path}"
                ...
                with ThreadPoolExecutor(max_workers=num_cores) as executor:
                    results = list(
                        tqdm(
                            executor.map(process_file, feat_files),
