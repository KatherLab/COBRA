...
def main_worker(gpu, ngpus_per_node, args, cfg):
    ...
    print(f"Initializing process group with {args.rank=}, {args.world_size=}, {args.gpu=}")
    ...
    model = MoCo(
        ...
        gpu_id=args.gpu,
        T=cfg["ssl"]["moco_t"],
        nr_mamba_layers=cfg["model"]["nr_mamba_layers"],
    ...
    #args.workers = int((cfg["ssl"]["workers"] + ngpus_per_node - 1) / ngpus_per_node) # ? round up or something
    args.workers = int(cfg["ssl"]["workers"]/args.world_size)
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(), args.lr, weight_decay=cfg["ssl"]["weight_decay"]
    )
    scaler = torch.amp.GradScaler("cuda")

    dataset = FeatDataset(
        pat_dict=get_pat_dict(cfg), num_feats=cfg["general"]["nr_feats"], feat_len=1536
    )
    ...
    for e in tqdm(
        range(args.start_epoch, cfg["ssl"]["epochs"]), disable=args.rank != 0
    ):
        t_loss = 0.0

        for i, (x1, sizes1, x2, sizes2) in enumerate(
            tqdm(loader, leave=False, disable=args.rank != 0)
        ):
    ...
    if epoch < cfg["ssl"]["warmup_epochs"]:
        ...
        lr = (
            ...
            * (
                ...
                + math.cos(
                    math.pi
                    * (epoch - cfg["ssl"]["warmup_epochs"])
                    / (cfg["ssl"]["epochs"] - cfg["ssl"]["warmup_epochs"])
                )
            )
        )
...
def adjust_moco_momentum(epoch, cfg):
    ...
    m = 1.0 - 0.5 * (1.0 + math.cos(math.pi * epoch / cfg["ssl"]["epochs"])) * (
        1.0 - cfg["ssl"]["moco_m"]
    )
    return m
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--config", type=str, default="config.yml", help="Path to the config file"
    )
    parser.add_argument(
        "--world-size",
        default=1,
    ...
    parser.add_argument(
        "--dist-url",
        default="env://",     #"tcp://localhost:23459",
        type=str,
